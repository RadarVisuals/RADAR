...
// --- IMPORT ABI ---
import { LSP8_ABI } from "../config/abis";
...
class TokenService {
  ...
  async initialize() {
    ...
    if (!this.collectionAddress || !isAddress(this.collectionAddress)) {
      if (import.meta.env.DEV)
        console.error("TokenService: Invalid or missing collection address.");
  ...
  async getOwnedTokenIds(userAddress) {
    ...
    try {
      ...
      if (import.meta.env.DEV)
        console.warn(
          `[TS] tokenIdsOf failed for ${this.collectionAddress}:`,
          error.message
        );
      return [];
    }
  ...
  async getAllLSP8TokenIdsForCollection(collectionAddress) {
    const checksummedAddr = collectionAddress
      ? getAddress(collectionAddress)
