...
export const useVisualEngine = () => {
    const { 
        stagedWorkspace, activeSceneName, isLoading, activeWorkspaceName, 
        setActiveSceneName, loadWorkspace, preloadWorkspace, setHasPendingChanges 
    ...
    const updateLayerConfig = useCallback((layerId, key, value, isMidiUpdate = false, skipStoreUpdate = false, isManual = false) => {
        ...
        if (engine) {
            // isManual=true causes an instant snap in the PixiLayerDeck
            engine.updateConfig(layerId, key, value, activeDeck, isManual);
        }
