...
.plock-midi-section .plock-controls-container {
  flex-grow: 1;
  margin-bottom: 0;
}
...
.slider-container.out-of-sync .horizontal-slider::-webkit-slider-thumb {
  background: var(--color-text-dim);
  box-shadow: none;
}
.slider-container.out-of-sync .horizontal-slider::-moz-range-thumb {
  background: var(--color-text-dim);
  box-shadow: none;
}
...
.slider-container.out-of-sync:hover .slider-value-indicator {
...
/* RECORDING State */
.plock-button.plock-record-button.recording {
  background-color: var(--color-error-a30);
  color: var(--color-error);
...
.plock-progress-bar {
  ...
  /* transition is not used, transform is set by requestAnimationFrame */
}
...
export const UserSessionProvider = ({ children }) => {
  ...
  const isRadarProjectAdmin = useMemo(() => {
    ...
    // --- START OF DEBUG LOG ---
    // This will print the values being compared to your browser's developer console.
    console.log(
      `%c[ADMIN CHECK]
      - VISITOR (You): ${visitorProfileAddress}
      - ADMIN   (Env): ${RADAR_OFFICIAL_ADMIN_ADDRESS}
      - MATCH: ${visitorProfileAddress?.toLowerCase() === RADAR_OFFICIAL_ADMIN_ADDRESS?.toLowerCase()}`,
