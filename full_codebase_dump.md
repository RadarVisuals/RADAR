...
.click-ping-svg-container {
  ...
  width: 40px; /* Increased size to contain animated circle */
  height: 40px; /* Increased size to contain animated circle */
  pointer-events: none; /* Allow clicks to pass through */
  z-index: 10001; /* Above canvases, potentially below some UI */
  opacity: 1;
  display: flex;
  justify-content: center;
  align-items: center;
 ...
 * If no visual preset is found, returns `config: null` but still attempts to load globals.
...
async loadConfiguration(profileAddress = null, configNameToLoad = null, customKey = null) {
  ...
  try {
      ...
      // Determine Target Config Key (if not custom)
      if (!targetConfigKey && !configNameToLoad) {
          ...
          if (nameFromPointer) {
              ...
              // console.log(`${logPrefix} Found default pointer: '${nameFromPointer}', Key: ${targetConfigKey}`);
          } else {
              // console.log(`${logPrefix} No default pointer found.`);
              configNameUsed = null;
              targetConfigKey = null; // Explicitly null if no default found
          }
          ...
          // console.log(`${logPrefix} Using provided name: '${configNameToLoad}', Key: ${targetConfigKey}`);
      } else if (targetConfigKey) {
           // console.log(`${logPrefix} Using custom key: ${targetConfigKey}`);
      }

      // Prepare Batch Read (Always include globals)
      ...
      if (dataKeysToFetch.length > 0) {
          try {
              // console.log(`${logPrefix} Fetching keys:`, dataKeysToFetch);
              ...
              // console.log(`${logPrefix} Batch read results (raw hex):`, dataValues);
          } catch (batchReadError) {
              console.error(`${logPrefix} Error during batch read via loadData:`, batchReadError);
              // Return default result structure on batch read error, keeping globals empty
              return { ...defaultResult, error: `Batch read failed: ${batchReadError.message}` };
          }
      } else {
           console.warn(`${logPrefix} No keys prepared for fetching.`); // Should not happen now
           return defaultResult;
      }

      // Parse Results
      let parsedConfig = null; // Initialize config as null
      let parsedReactions = {};
      let parsedMidi = {};

      // Parse Config (Only if a key was targeted and data exists)
          ...
          if (configHex && configHex !== "0x") {
              ...
              if (configJson) {
                  try {
                      ...
                      // Basic validation: check for layers and tokenAssignments keys
                      if (tempParsed && typeof tempParsed === "object" && typeof tempParsed.l === 'object' && typeof tempParsed.tA === 'object') {
                          ...
                          // console.log(`${logPrefix} Successfully parsed config: '${parsedConfig.name}'`);
               ...
               // console.log(`${logPrefix} No data found for config key ${targetConfigKey}.`);
          }
      } else {
           // console.log(`${logPrefix} No targetConfigKey was determined, skipping config parse.`);
      }

      // Parse Reactions (Always attempt)
      if (keyIndexMap.reactions !== -1) {
          const reactionsHex = dataValues[keyIndexMap.reactions];
           if (reactionsHex && reactionsHex !== "0x") {
               ...
               if (reactionsJson) {
                  try {
                      ...
                      if (tempParsed && typeof tempParsed === "object") {
                          ...
                          // console.log(`${logPrefix} Successfully parsed reactions.`);
               ...
               // console.log(`${logPrefix} No data found for reactions key.`);
          }
      }

      // Parse MIDI (Always attempt)
      if (keyIndexMap.midi !== -1) {
          const midiHex = dataValues[keyIndexMap.midi];
           if (midiHex && midiHex !== "0x") {
               ...
               if (midiJson) {
                  try {
                      ...
                      if (tempParsed && typeof tempParsed === "object") {
                          ...
                          // console.log(`${logPrefix} Successfully parsed MIDI map.`);
               ...
               // console.log(`${logPrefix} No data found for MIDI key.`);
          }
      }

      // Return the combined result. `config` will be null if no specific preset was found/loaded.
      // console.log(`${logPrefix} Load complete. Returning:`, { config: parsedConfig, reactions: parsedReactions, midi: parsedMidi });
